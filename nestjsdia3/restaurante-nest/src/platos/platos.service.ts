...
export class PlatosService {
  constructor(
    @InjectModel(Plato.name)
    private platoModel: Model<PlatoDocument>,
  ) {}
  ...
  async create(createPlatoDto: any) {
    const plato = new this.platoModel(createPlatoDto);
    return plato.save();
  }

  async update(id: string, updatePlatoDto: any) {
    const plato = await this.platoModel
      .findByIdAndUpdate(id, updatePlatoDto, { new: true })
      .exec();
    if (!plato) throw new NotFoundException(`Plato ${id} no encontrado`);
    return plato;
  }
